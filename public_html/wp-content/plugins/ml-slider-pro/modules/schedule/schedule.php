...
class MetaSliderPro_Schedule_Slides
{
    ...
    public function schedule_slides()
    {
        ...
        if (class_exists('Meta_Slider_Schedule_Slides') && method_exists(
            ...
            if (is_admin() && current_user_can('activate_plugins') && is_plugin_active($schedule_slides_standalone)) {
                ...
                new Metaslider_Admin_Notice(
                    'notice-error',
                    __(
                        'Plugin <strong>Deactivated</strong>! MetaSlider Schedule Slides is no longer needed as we have added its functionality to MetaSlider - Pro Add-on Pack. Activating the plugin will cause a conflict on your site.',
                        'ml-slider-pro'
                    )
                );
